...
Deletes (for --date, default today):
  ...
  jobs/jobs_<date>_*.md (job list + visa file), jobs/research_<date>_sponsors.md, jobs/unclear_<date>.md,
...
Never deletes: code, the resume, .env, companies.yaml, applications/, .claude/, JOB_RESULTS.md,
  jobs/seen.json, jobs/discovered_companies.yaml, jobs/visa_companies.json
...
def targets(date: str) -> list[Path]:
    out = [JOBS / "jd", JOBS / "daily", JOBS / "latest.md",
           *JOBS.glob(f"jobs_{date}_*.md"),
           *(JOBS / n for n in (f"research_{date}_sponsors.md", f"unclear_{date}.md", f"unclear_visa_match_{date}.md",
                                f"visa_jobs_{date}.json", f"visa_research_{date}.json"))]
...
def move_jds(date: str, dry: bool) -> None:
    ...
    for f in sorted(day.glob("*.md")):
        ...
        if not dry:
            ...
            text = text.replace(f"(jobs/jd_visa/{date}/{f.name})", f"(applications/{date}/{f.stem}/job_description.md)")
    if not dry and text:
